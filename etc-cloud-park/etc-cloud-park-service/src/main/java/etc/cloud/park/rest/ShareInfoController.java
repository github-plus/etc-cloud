...
import etc.cloud.park.service.impl.ShareInfoServiceImpl;
...
@RestController
@RequestMapping("/etc-cloud/share_info")
@CrossOrigin
public class ShareInfoController {

    @Autowired
    private ShareInfoServiceImpl shareInfoService;
    ...
    public Object FindAll(@RequestBody(required = false) String data)
    {
        //获取token
        ...
        return shareInfoService.ShareInfo(data);
    }

    @PostMapping("/select_wrapper")
    public Object shareSelect(@RequestBody(required = false) String data)
    {
        //获取token
        String token = request.getHeader("X-Token");
        ...
        Boolean menu = loginFilterService.findMenu(account, 11);
        if(menu == null) return MsgResponse.error().msg("该账号没有这个权限");
        return shareInfoService.ShareSelect(data);
    }
}
